document.addEventListener('DOMContentLoaded', () => {
  ...
  async function getPageSource(tabInfo, d) {
    ...
    const result = await chrome.scripting.executeScript({
        ...
        function getXPath(element) {
          if (element && element.nodeType === Node.ELEMENT_NODE) {
            ...
            while (element && element.nodeType === Node.ELEMENT_NODE) {
              ...
              if (element.id) {
                path.unshift(`/*[@id="${element.id}"]`);  // Modified line: Prepend `//*[@id=""]` instead of `//id("")`
                break;
              } else {
                let sibling = element;
                let index = 1;
                while (sibling && sibling.nodeType === Node.ELEMENT_NODE) {
                  sibling = sibling.previousElementSibling;
                  if (sibling && sibling.nodeName.toLowerCase() === tagName) {
                    index++;
                  }
                }
                if (index > 1) {
      ...
      // Store the clicked element in the clickedElements array
          ...
          const actual_xpath=getXPath(event.target)
          
const result = document.evaluate(actual_xpath, document, null, XPathResult.ANY_TYPE, null);
...
if (node) {
  ...
  node.style.color = 'red';
          ...
          fetch("http://127.0.0.1:8000/capture_xpath", {
            method: "POST",
            body: JSON.stringify({ xpath: xpath,xpath1:xpath1 ,actual_xpath:actual_xpath})
          })
          ...
          console.log('Clicked actual XPath:', actual_xpath);
